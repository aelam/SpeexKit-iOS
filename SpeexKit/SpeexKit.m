...
-(void) startEncode:(char*)inFile outFile:(char*)outFile
    {
        ...
        int chan=1;
        int fmt=16;
        spx_int32_t quality=1;//[[JDCore defaultCore] voiceEncodingQuality];
        float vbr_quality=-1;
        int lsb=1;
        ogg_stream_state os;
...
//                    exit(1);
        ...
        if (!wave_input)
        {
            ...
            nb_samples=[self read_samples:fin frame_size:frame_size bits:fmt channels:chan lsb:lsb input:input buff:first_bytes size:NULL];
        } else {
            
            nb_samples=[self read_samples:fin frame_size:frame_size bits:fmt channels:chan lsb:lsb input:input buff:NULL size:&size];
        }
        if (nb_samples==0)
            eos=1;
        ...
        while (!eos || total_samples>nb_encoded)
        {
            ...
            if (wave_input)
            {
                ...
                nb_samples=[self read_samples:fin frame_size:frame_size bits:fmt channels:chan lsb:lsb input:input buff:NULL size:NULL];
            }
            if (nb_samples==0)
            {
                eos=1;
